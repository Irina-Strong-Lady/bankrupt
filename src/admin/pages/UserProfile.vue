    ...
    <div class="wrapper">
      ...
      <div class="caption-wrapper">
        ...
        <transition name="fade">
          <h3 
            v-if="userData.admin && 
                  userData.name != profileStore?.usersArray[profileStore.userIdx]?.name"
            class="edit">
            Профиль пользователя "{{ profileStore?.usersArray[profileStore.userIdx]?.name }}" в процессе редактирования
            <span style="width: .75em; color: white; display: inline-block">
          ...
          <tbody>
            <tr>
              <div class="row-wrapper">
                <th>Имя:</th>
                <td v-if="userData.admin">{{ userData.name == profileStore?.usersArray[profileStore.userIdx]?.name ? 
                      userData.name : 
                      profileStore?.usersArray[profileStore.userIdx]?.name }}
                </td>
            ...
            <tr>
              <div class="row-wrapper">
                <th>Телефон:</th>
                <td v-if="userData.admin">{{ userData.phone == profileStore?.usersArray[profileStore.userIdx]?.phone ? 
                       userData.phone : 
                       profileStore?.usersArray[profileStore.userIdx]?.phone }}
                </td>
              ...
              <transition 
                ...
                <div v-if="selectRoleVisible" :class="['btn-wrapper', { selectProfileVisible: 'btn-transition'}]">
                  <el-select
                    ...
                    :placeholder="userData.admin && profileStore?.usersArray[profileStore.userIdx]?.admin ? 
                                  'Администратор' : 
                                  'Исполнитель'" 
                    filterable
                    allow-create
